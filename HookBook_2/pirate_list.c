...
pirate_list* list_create()
{
    ...
    pirates->list_pirate = calloc(INITIAL_CAPACITY, sizeof(pirate*));
    pirates->length = 0;
...
pirate *list_remove(pirate_list *pirates, pirate *p)
{
    ...
    {
        //pirate *index = pirates->list_pirate[list_index_of(pirates, p)];
...
void list_sort(pirate_list *pirates, char* type)
{
    ...
    for(int i = 1; i < pirates->length; i++)
    {
        ...
        while (j >= 0 && choose_sort(pirates->list_pirate[j], temp, type) > 0) 
        {
